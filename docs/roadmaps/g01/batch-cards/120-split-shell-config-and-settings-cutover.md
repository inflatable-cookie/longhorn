# 120 Split-shell Config And Settings Cutover
...
Move Split-shell's single workspace preference document onto the smallest useful
...
- Split-shell: only config, settings, dependency, test, and migration files named by
  the card after unrelated-work clearance.
- Poodle: read-only exact artifact use.

## Scope

- canonical id `com.example.split-shell`
...
1. Recheck Split-shell status and exclude every unrelated dirty docs path.
...
- unrelated Split-shell work is byte-for-byte preserved
...
Split-shell now resolves `platform-native-v1` from canonical id
`com.example.split-shell` with no stable storage leaf. One versioned
...
The modal registry contains only Split-shell Workspace and shared Storage pages.
...
Split-shell `effigy qa` passes with 60 Rust tests and zero Svelte diagnostics. The
...
Execute Card 121. Close Split-shell as the minimal real consumer and prove the
