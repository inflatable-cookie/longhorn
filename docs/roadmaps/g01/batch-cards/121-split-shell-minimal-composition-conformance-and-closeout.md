...
- fresh, legacy, corrupt, interrupted, restart, and rollback preference paths
- modal settings session and storage diagnostics
- selected workspace, expanded nodes, selection, and split-ratio preservation
- public Poodle component graph and Svelte lifetime
- exact package, capability, source, artifact, and duplicate inventories
- explicit no-layout and no-optional-system proof
...
6. Remove the superseded generic `workspace.json` writer.
7. Verify content and Git authorities are unchanged.
...
- conformance requires hiding unsupported content
- a second preference authority remains active
- minimal composition resolves an unselected package
- closeout alters authored workspace content

## Next Task

Execute Card 122. Adopt Jetstream's typed bridge and shared command/keymap
mechanisms while keeping product execution local.
