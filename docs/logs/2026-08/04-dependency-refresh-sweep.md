...
- **zip `=5.1.1` → `=8.6.0` — bumped.** Zero source changes; the
  deflate-flate2-zlib-rs feature survives three majors. All backup-archive
  publication, determinism, retention, and restore suites green. Reached
  8.x only after the toolchain floor rose to 1.90 later the same day; the
  sweep itself had stopped at 7.2.0, the 1.85 ceiling.
...
- `cargo update` applied. The sweep first pinned darling, plist,
  serde_with, time, idna_adapter, writeable, and the icu 2.2 family back to
  1.85-compatible versions; the 1.90 floor released all seven, leaving
  seven transitive crates behind latest for upstream range reasons alone
