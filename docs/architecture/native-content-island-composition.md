...
Card 093 packs `@longhorn/core`, `@longhorn/native-content`, and
`@longhorn/native-content-svelte`, inventories the five Rust crates, and runs
...
| --- | --- | --- | --- |
| Nucleus | pass | Card 094 read-only freeze | published prerelease, public Poodle visibility seam, and Nucleus g05 acceptance |
...
checked evidence before the published-artifact and consumer gates are reviewed.
...
`@longhorn/native-content` owns no mechanism. Its root accepts injected direct
...
Card 092 adds `@longhorn/native-content-svelte` as a separate package over the
