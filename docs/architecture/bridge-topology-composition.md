...
| --- | --- | --- | --- |
| Direct in-process domain | `longhorn-bridge` | `@longhorn/bridge` | none |
...
`@longhorn/bridge` depends on `@longhorn/core` and `@longhorn/tauri`.
...
- `@longhorn/bridge/tauri` for invoke
- `@longhorn/bridge/tauri-events` only for ordered domain or job events
...
`proof:bridge-topology-artifacts` packs `@longhorn/core`,
`@longhorn/tauri`, and `@longhorn/bridge`, then installs five clean consumers.
