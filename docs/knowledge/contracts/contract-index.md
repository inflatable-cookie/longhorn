...
- Contract 022 was promoted 2026-08-19 from the agent-control spike (research
  memo 024, in Git history): stateless mount and 2026-07-28 negotiation
  proved on the wire, unfocused/occluded/minimized capture proved fresh, with
  the rAF/timer caveat folded into the contract's `wait_for` semantics. A
  consumer may ship it in a packaged build with `evaluate` omitted. The
  2026-09-26 amendment admits Rust-side pickers through a consumer-carried
  origin and HTML file inputs through an inline-bytes tool.
- Contract 023's production MCP role is withdrawn (operator direction
  2026-09-22). The contract 022 opt-in server is the production MCP. Longhorn
  keeps only the typed validation and dispatch record.
- Contract 012's publication clause is live since `0.1.0` (2026-09-16).
