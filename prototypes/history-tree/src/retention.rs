...
use crate::{ForkBranchId, ForkCheckpointId, ForkHistory};

/// Count and encoded-weight budgets for private graph pruning.
...
impl ForkRetentionLimits {
    /// Validates nonzero retention limits.
    pub const fn new(
        maximum_entries: usize,
        maximum_encoded_weight: u64,
...
impl ForkPruningReceipt {
    ...
    /// Returns opaque checkpoint references invalidated by pruning.
    #[must_use]
...
impl<P> ForkHistory<P> {
    ...
    pub fn prune_to(
        ...
        for entry_id in prune_ids {
            ...
            if let Some(children) = self.children.get_mut(&node.parent_entry_id().cloned()) {
    ...
    fn protected_lineage(&self) -> Result<BTreeSet<HistoryEntryId>, ForkRetentionError> {
        ...
        for branch in self.branches.values().filter(|branch| {
            ...
            protected.extend(
                self.lineage::<ForkRetentionError>(branch.head_entry_id())
                    .map_err(|_| ForkRetentionError::InvalidTopology)?,
            );
        }
        protected.extend(
            self.lineage::<ForkRetentionError>(self.current_node_id.as_ref())
                .map_err(|_| ForkRetentionError::InvalidTopology)?,
        );
        Ok(protected)
    }
...
/// Rejected private graph retention operation.
#[derive(Clone, Debug, Eq, PartialEq)]
