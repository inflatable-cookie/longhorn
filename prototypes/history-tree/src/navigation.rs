...
use longhorn_history::{HistoryNavigationStep, HistoryNavigationTransactionFailure, HistoryPolicy};

use crate::{ForkBranchId, ForkHistory};

/// Private graph navigation intent.
...
/// Immutable mixed undo/redo LCA route.
#[derive(Clone, Debug, Eq, PartialEq)]
...
/// Consumer-owned atomic product transaction for a private graph route.
...
impl<P: Clone> ForkHistory<P> {
    /// Plans a private undo, preferred redo, or branch checkout.
        ...
        T: HistoryPolicy<P>,
    {
        ...
        let source_lineage = self.lineage(self.current_node_id.as_ref())?;
        let target_lineage = self.lineage(target_node_id.as_ref())?;
        ...
        for entry_id in source_lineage[shared..].iter().rev() {
            let node = self
                .nodes
                .get(entry_id)
                .expect("lineage contains registered nodes");
            let payload =
                policy
...
impl<P> ForkHistory<P> {
    /// Revalidates, atomically applies, then commits one private route.
        ...
        T: ForkNavigationTransaction<P>,
    {
        ...
        let moved_entry_ids = plan
            ...
            .collect();
        let target_lineage = match self.lineage::<T::Error>(plan.target_node_id.as_ref()) {
            Ok(lineage) => lineage,
            Err(_) => unreachable!("validated target lineage"),
        };
        let shared = plan
            .lowest_common_ancestor
            .as_ref()
        ...
        match target {
            ForkNavigationTarget::Undo => {
                ...
                let parent = self
                    .nodes
                    .get(current)
                    .expect("current node is registered")
                    .parent_entry_id()
                    .cloned();
    ...
    fn route_ids<E>(
        ...
        let source_lineage = self.lineage(source)?;
        let target_lineage = self.lineage(target)?;
        let shared = source_lineage
            .iter()
            .zip(&target_lineage)
            .take_while(|(left, right)| left == right)
    ...
    pub(crate) fn lineage<E>(
        ...
        while let Some(entry_id) = cursor {
            if !seen.insert(entry_id.clone()) {
                return Err(ForkNavigationError::Cycle(entry_id));
            }
            let node = self
                .nodes
                .get(&entry_id)
...
/// Rejected private graph navigation.
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum ForkNavigationError<E> {
    ...
    /// Private plan no longer matches graph state.
    InvalidPlan,
    /// Imported or corrupted topology contains a cycle.
    Cycle(HistoryEntryId),
