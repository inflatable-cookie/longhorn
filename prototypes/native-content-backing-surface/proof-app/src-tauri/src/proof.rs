...
use longhorn_native_content_backing_surface_prototype::{
    AdapterEvent, BackingSurfaceAdapter, BackingSurfaceError, BackingSurfaceSpec, DetachOutcome,
    InputAdmission, InputRejection, RuntimeEvent, RuntimeEventKind,
};
use longhorn_native_content_prototype::{
    ...
    NativeContentCoordinator, NativeContentIslandId, NativeContentKindId, NativeContentMechanism,
...
const GENERATION: AttachGeneration = AttachGeneration::new(1);
...
pub(crate) fn run(
    ...
    let events = Arc::new(Mutex::new(Vec::<AdapterEvent>::new()));
    let runtime = TauriBackingRuntime::new(app.clone(), "controller", DetachPolicy::Reversible);
    ...
    let initial_receipt = adapter.apply(&initial_plan).map_err(string_error)?;
    ...
    checks.push(Check::new(
        ...
        status(
            std::env::consts::OS == "macos"
                ...
                && initial.native_view_attached,
        ),
        json!({
            "platform": std::env::consts::OS,
            "native_view_attached": initial.native_view_attached,
            "transparent_webview": true,
            "native_order": "NSView below webview",
            "controlled_fixture": "full-host NSView plus deterministic renderer output view",
        }),
    ));
    checks.push(Check::new(
    ...
    checks.push(Check::new(
        ...
        status(
            collapsed.snapshot.clip.size().is_empty()
                ...
                && collapsed.snapshot.native_view_attached
                && collapsed_input == InputAdmission::Rejected(InputRejection::EmptyViewport)
                && restored.snapshot.native_view_attached
    ...
    checks.push(Check::new(
        ...
        status(
            inside == InputAdmission::Admitted
                ...
                && *semantic_callbacks.lock().map_err(|_| "semantic callback store is poisoned".to_string())?
    ...
    checks.push(Check::new(
        ...
        json!({
            ...
            "native_focus_observation": "unknown; host focus is injected policy evidence only",
        }),
    ));
    ...
    checks.push(Check::new(
        ...
        status(
            after_resize.storage_bounds != before_resize.storage_bounds
                ...
                && after_resize.native_view_attached,
        ),
        json!({"before": before_resize, "after": after_resize}),
    ));

    let scale_check = exercise_available_scale(
        &window,
        &adapter,
        &runtime,
        &mut coordinator,
    ...
    let stale_plan_error = adapter.apply(&stale_move_plan).unwrap_err();
    let stale_event_error = adapter
        .admit_runtime_event(RuntimeEvent {
            island_id: island_id()?,
            host_window_id: host_window_id()?,
            generation: AttachGeneration::new(0),
            kind: RuntimeEventKind::FramePresented { sequence: u64::MAX },
        })
        .unwrap_err();
    ...
    checks.push(Check::new(
        ...
        status(
            matches!(stale_plan_error, BackingSurfaceError::StalePlan { .. })
                && matches!(
                    stale_event_error,
    ...
    let invalidated = adapter
        .host_destroyed(&host_window_id()?)
        .map_err(string_error)?
        .ok_or_else(|| "host destruction did not invalidate the attachment".to_string())?;
    let late = adapter
        .admit_runtime_event(RuntimeEvent {
            island_id: island_id()?,
            host_window_id: host_window_id()?,
    ...
    checks.push(Check::new(
        ...
        status(
            invalidated.detach_outcome() == DetachOutcome::Detached
                && invalidated_index
                    .zip(detached_index)
                    .is_some_and(|(left, right)| left < right)
                && matches!(late, BackingSurfaceError::NotAttached),
        ),
        json!({
            "invalidated_generation": invalidated.generation(),
            "detach_outcome": invalidated.detach_outcome(),
    ...
    if report.failed() {
        return Err("one or more backing-surface proof checks failed".to_string());
    }
    app.exit(0);
    Ok(())
}
...
struct Transition {
    plan: ApplyPlan,
    snapshot: longhorn_native_content_backing_surface_prototype::RuntimeSnapshot,
...
fn exercise_available_scale(
    ...
    if (transitioned_native - current_native).abs() <= f64::EPSILON {
        ...
        return Ok(Check::new(
            ...
            CheckStatus::Unmet,
            json!({
                "reason": "a distinct-scale monitor exists but the packaged window did not transition",
                "current_native_scale": current_native,
...
fn boundary_audit() -> Check {
    let adapter_source = concat!(
        include_str!("../../../adapter/src/adapter.rs"),
        include_str!("../../../adapter/src/runtime.rs"),
        include_str!("../../../adapter/src/lib.rs"),
    )
    .to_ascii_lowercase();
    let adapter_manifest = include_str!("../../../adapter/Cargo.toml").to_ascii_lowercase();
    ...
    Check::new(
        ...
        json!({
            ...
            "generic_pointer_payload_in_adapter": false,
            "native_handle_exposed_by_adapter": false,
            "poodle_private_dom_inspection": false,
            "visual_chrome": "ordinary controlled HTML/CSS fixture",
            "renderer_fixture_location": "proof app only",
        }),
    )
}
...
fn island_id() -> Result<NativeContentIslandId, String> {
    NativeContentIslandId::new("island:backing-surface-proof").map_err(string_error)
}

fn host_window_id() -> Result<WindowId, String> {
    WindowId::new("window:backing-surface-proof").map_err(string_error)
}
