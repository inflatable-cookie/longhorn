...
use longhorn_native_content_isolated_window_prototype::{
    ...
    ProcessIsolatedWindowRuntime, ProcessRuntimeConfig, TeardownMode, TeardownOutcome,
};
use longhorn_native_content_prototype::{
    ...
    DesiredUpdate, DesiredVisibility, DetachPolicy, FocusIntent, InputRoutingMode,
    MechanismCapabilities, NativeContentCoordinator, NativeContentFailureCode,
    NativeContentIslandId, NativeContentKindId, NativeContentMechanism, OperationOutcome,
...
type RuntimePort = ProcessIsolatedWindowRuntime;
type Adapter = IsolatedWindowAdapter<RuntimePort>;
...
pub(crate) fn run<R: Runtime>(
    ...
    let adapter = IsolatedWindowAdapter::new(
        runtime.clone(),
        IsolatedWindowSpec::new(island_id()?, host_window_id()?, TEARDOWN_WAIT),
        Arc::new(move |event| {
    ...
    let real_child = log.wait_for("adapter_event", WAIT, |detail| {
        detail["kind"] == "runtime"
            ...
            && detail["event"]["native_child_attached"] == true
    })?;
    ...
    adapter
        .script_request(AttachGeneration::new(1), ChildRequest::Hide)
        .map_err(string_error)?;
    ...
    adapter
        .script_request(AttachGeneration::new(1), ChildRequest::Show)
        .map_err(string_error)?;
    ...
    adapter
        ...
        .map_err(string_error)?;
    adapter
        .set_resizable(AttachGeneration::new(1), true)
        .map_err(string_error)?;
    checks.push(Check::new(
    ...
    adapter
        .script_request(AttachGeneration::new(1), ChildRequest::Close)
        .map_err(string_error)?;
    ...
    let attached_two = adapter
        .observe(AttachGeneration::new(2))
        .map_err(string_error)?;
    generation_two
    ...
    runtime
        ...
        .map_err(string_error)?;
    let timed_out = apply(&adapter, &detach_plan, "teardown_timeout", &log)?;
    runtime
        .set_teardown_mode(TeardownMode::TerminateOwner)
        .map_err(string_error)?;
    let terminated = apply(&adapter, &detach_plan, "owner_process_termination", &log)?;
    ...
    let stale = adapter.admit_runtime_event(HelperEvent {
        ...
        kind: HelperEventKind::ChildRequest {
...
fn observe(
    ...
    let observation = adapter
        .observe(AttachGeneration::new(generation))
        .map_err(string_error)?;
    let value = serde_json::to_value(observation).map_err(string_error)?;
    log.record(
        "native_observation",
