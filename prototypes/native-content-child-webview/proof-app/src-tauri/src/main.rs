//! Packaged native proof for Longhorn's private child-webview mechanism prototype.
...
fn setup(app: &mut tauri::App<Wry>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let host = WindowBuilder::new(app, "host")
        .title("Longhorn Child Webview Packaged Proof")
    ...
    thread::spawn(move || {
        ...
        if let Err(error) = proof::run(app_handle.clone(), host, log.clone()) {
...
fn evidence_root(app: &tauri::App<Wry>) -> Result<PathBuf, Box<dyn std::error::Error>> {
    if let Some(root) = std::env::var_os("LONGHORN_CHILD_WEBVIEW_EVIDENCE_DIR") {
...
fn main() {
    tauri::Builder::default()
        ...
        .expect("could not run packaged Longhorn child-webview proof");
}
