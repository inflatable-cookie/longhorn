...
import { ForkHistoryClient } from "../src/client.ts";
import { SerializedForkHistoryPort } from "../src/serialized.ts";
import type { ForkHistoryPort } from "../src/ports.ts";
import { branchPage, changed, clone, committed, pathPage, snapshot } from "./support.ts";

function port(): ForkHistoryPort { return { snapshot: async () => clone(snapshot), path: async () => clone(pathPage), branches: async () => clone(branchPage), navigate: async () => clone(committed), listen: (listener) => { listener(clone(changed)); return () => {}; }, nextPlanId: () => "plan:test" }; }
