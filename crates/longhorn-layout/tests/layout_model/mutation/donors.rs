...
use longhorn_layout::{
    ...
    LayoutMutationCommand, LayoutMutationEngine, LayoutMutationRequest, LayoutSchemaDefinition,
    PanelDefinition, PanelInstancePolicy, PlacementSelector, RegionDefinition, RegionState,
    SizingSlotDefinition, SizingSlotState,
};
...
fn run_donor_sequence<const REGIONS: usize, const SLOTS: usize>(
    ...
    assert_eq!(sized.committed_revision(), LayoutRevision::new(6));
    assert_eq!(
        sized.authoritative_document().containers()[0]
...
fn donor_document(schema: &str, regions: &[&str], slots: &[&str]) -> LayoutDocument {
    LayoutDocument::new(
...
) -> longhorn_layout::LayoutMutationReceipt {
    engine
        .apply(
            document,
