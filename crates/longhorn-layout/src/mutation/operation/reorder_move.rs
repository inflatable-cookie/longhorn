...
pub(super) fn reorder_region(
    ...
    *region_mut(document, container_id, region_id)?.panel_instance_ids_mut() =
...
pub(super) fn move_panel(
    ...
    if source.container_id == *target_container_id && source.region_id == *target_region_id {
        return Err(operation_rejection(
            LayoutMutationRejectionCode::MoveTargetUnchanged,
            "same-region position changes require a complete reorder command",
        ));
    }
    let target_container = document.container(target_container_id).ok_or_else(|| {
        operation_rejection(
            LayoutMutationRejectionCode::UnknownContainer,
            format!("unknown layout container {target_container_id}"),
        )
    })?;
    let target_region = target_container.region(target_region_id).ok_or_else(|| {
        operation_rejection(
            LayoutMutationRejectionCode::UnknownRegion,
            format!("container {target_container_id} has no region {target_region_id}"),
        )
    })?;
    ...
    let source_region = region_mut(document, &source.container_id, &source.region_id)?;
    remove_with_active_fallback(source_region, panel_instance_id, source.index);
    let target_region = region_mut(document, target_container_id, target_region_id)?;
    target_region
    ...
    Ok(LayoutMutationOutcome::PanelMoved {
        ...
        target_container_id: target_container_id.clone(),
