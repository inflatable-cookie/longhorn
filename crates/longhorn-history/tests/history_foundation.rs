//! Contract fixtures for the typed linear history foundation.

#[path = "history_foundation/document.rs"]
mod document;
#[path = "history_foundation/grouping.rs"]
mod grouping;
#[path = "history_foundation/invariants.rs"]
mod invariants;
#[path = "history_foundation/navigation_failures.rs"]
mod navigation_failures;
#[path = "history_foundation/navigation_pulse_shaped.rs"]
mod navigation_pulse_shaped;
#[path = "history_foundation/persistence.rs"]
mod persistence;
#[path = "history_foundation/persistence_fuzz.rs"]
mod persistence_fuzz;
#[path = "history_foundation/persistence_pulse_shaped.rs"]
mod persistence_pulse_shaped;
#[path = "history_foundation/projection.rs"]
mod projection;
#[path = "history_foundation/protocol.rs"]
mod protocol;
#[path = "history_foundation/pulse_shaped.rs"]
mod pulse_shaped;
#[path = "history_foundation/retention.rs"]
mod retention;
#[path = "history_foundation/support.rs"]
mod support;
#[path = "history_foundation/transitions.rs"]
mod transitions;
