...
fn keyboard_case_with_reserved(
    keymap: &CommandEffectiveKeymap,
    platform: CommandPlatform,
    input: CommandKeyboardInput,
    context: &CommandContextSnapshot,
...
fn authority() -> Result<(longhorn_command::CommandRegistry, CommandKeymapPreset), Box<dyn Error>> {
    let mut builder =
...
fn binding(id_value: &str, code: &str, command: &str) -> CommandBindingDefinition {
...
fn binding_id(value: &str) -> CommandBindingId {
    id(value)
}
