...
pub fn variant_field_map(
    ...
    for declaration in declarations {
        ...
        let variants = tagged_field_map(declaration, tag);
        if variants.is_empty() {
            // A union whose arms this splitter cannot read, or one tagged on a
            // different key. Reported rather than skipped silently, because a
...
branchId: BranchId, };";
    ...
    fn a_nullable_field_does_not_make_a_record_a_union() {
        ...
        let (rendered, unreadable) =
            variant_field_map("test", "MAP", "kind", &[declaration.to_owned()]);
