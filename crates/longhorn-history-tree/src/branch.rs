...
/// Hard limit for one branch name.
pub const MAXIMUM_FORK_BRANCH_NAME_BYTES: usize = 256;
/// Hard limit for one branch annotation.
...
impl ForkBranchMetadata {
    ...
    /// Returns whether later retention must protect this branch.
    #[must_use]
...
pub enum ForkBranchMetadataError {
    ...
    /// An optional value exceeded its hard byte limit.
    TooLong {
        /// Field name.
        field: &'static str,
...
impl fmt::Display for ForkBranchMetadataError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Empty { field } => write!(formatter, "fork branch {field} cannot be empty"),
            Self::TooLong {
                field,
                maximum,
                actual,
                ...
                formatter,
                "fork branch {field} is {actual} bytes; maximum is {maximum}"
            ),
        }
    }
}

impl Error for ForkBranchMetadataError {}
...
impl ForkBranch {
    /// Constructs a branch for validated structural-state import.
    #[must_use]
    pub const fn new(
        branch_id: ForkBranchId,
        head_entry_id: Option<HistoryEntryId>,
    ...
    /// Returns the current branch head, or the graph root.
    #[must_use]
    pub const fn head_entry_id(&self) -> Option<&HistoryEntryId> {
        self.head_entry_id.as_ref()
    }

    /// Returns mutable metadata owned by the branch reference.
    #[must_use]
    pub const fn metadata(&self) -> &ForkBranchMetadata {
        &self.metadata
    }

    pub(crate) fn set_head(&mut self, head_entry_id: HistoryEntryId) {
