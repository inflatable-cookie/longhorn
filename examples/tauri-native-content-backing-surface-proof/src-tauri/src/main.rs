...
fn main() {
    tauri::Builder::default()
        .setup(|app| {
            ...
            log.record(
                ...
                json!({
                    ...
                    "fixture": "production adapter, full-host NSView, deterministic consumer renderer",
        ...
        .expect("could not run packaged Longhorn backing-surface production proof");
}

fn evidence_root(app: &tauri::App<tauri::Wry>) -> Result<PathBuf, Box<dyn std::error::Error>> {
    if let Some(root) = std::env::var_os("LONGHORN_BACKING_SURFACE_PRODUCTION_EVIDENCE_DIR") {
