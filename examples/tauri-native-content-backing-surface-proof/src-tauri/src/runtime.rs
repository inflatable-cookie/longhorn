...
use longhorn_native_content::InputRoutingMode;
use longhorn_native_content_backing_surface::{
    BackingSurfaceError, BackingSurfaceRuntime, BackingSurfaceRuntimeEvent, BackingSurfaceSnapshot,
...
struct Session {
    ...
    #[allow(dead_code)]
    callback: Arc<dyn Fn(BackingSurfaceRuntimeEvent) + Send + Sync>,
}

impl TauriBackingRuntime {
    pub(crate) fn new(app: AppHandle<Wry>, window_label: impl Into<String>) -> Self {
        Self {
            app,
    ...
    fn native_state(
        &self,
        handle: u64,
    ) -> Result<(NativeToken, BackingSurfaceSnapshot), BackingSurfaceError> {
        self.state
            .lock()
            .map_err(|_| BackingSurfaceError::Poisoned)?
            .sessions
...
impl BackingSurfaceRuntime for TauriBackingRuntime {
    ...
    fn observe(
        ...
        session.snapshot.native_storage_attached = evidence.attached;
        session.snapshot.native_scale = scale_factor(scale)?;
        Self::render(session);
        Ok(session.snapshot.clone())
    }
    ...
    fn detach(&self, handle: &Self::Handle) -> Result<(), BackingSurfaceError> {
        let session = self
            ...
            .ok_or(BackingSurfaceError::NotAttached)?;
        let window = self.window()?;
        on_main(window, move |_| native_macos::detach(session.native, true))
    }
}
