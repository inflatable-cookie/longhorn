...
async function verifyTypescriptGraph(
  ...
  await writeFile(join(stage, "package.json"), stableJson({
    name: `longhorn-card127-${definition.name}`,
    private: true,
    type: "module",
    dependencies,
...
async function verifyRustGraphs(
  ...
  await run(["cargo", "+1.95.0", "update", "-p", "tauri", "--precise", "2.11.5", "--offline"], workspace);
  const graphs = new Map<string, string[]>();
  for (const definition of definitions) {
    const packageName = `longhorn-card127-${definition.name}`;
    await run(["cargo", "+1.95.0", "check", "-p", packageName, "--locked", "--offline"], workspace);
    const tree = await run(["cargo", "+1.95.0", "tree", "-p", packageName, "--locked", "--offline", "--edges", "normal", "--prefix", "none"], workspace);
    const resolved = [...new Set(tree.split("\n").map((line) => line.trim().split(/\s+/)[0] ?? "").filter((name) => name.startsWith("longhorn-") && !name.startsWith("longhorn-card127-")))].sort();
