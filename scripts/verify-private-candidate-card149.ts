...
import {
  ...
  produceArtifacts,
  rustPackages,
} from "./private-candidate-card149/artifacts";
import {
  assertGraphSourcesClean,
  graphDefinitions,
  verifyConsumerGraphs,
} from "./private-candidate-card149/consumers";
import {
  candidateVersion,
  digest,
  equal,
  ...
  run,
  stableJson,
} from "./private-candidate-card149/support";
...
const fixturePath = join(repoRoot, "fixtures/release/card149/private-0-1-candidate-v2.json");
...
const temporaryRoot = await mkdtemp(join(tmpdir(), "longhorn-card149-candidate-"));
...
try {
  ...
  const report = {
    ...
    tools: {
      rust: "1.85.0",
      rustc: (await run(["rustc", "+1.85.0", "--version"])).trim(),
      tauriRust: "2.11.5",
  ...
  if (write) {
    await mkdir(join(repoRoot, "fixtures/release/card149"), { recursive: true });
