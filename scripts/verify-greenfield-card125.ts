...
async function verifyRustArtifacts() {
  ...
  for (const shape of Object.keys(policies) as ShapeName[]) {
    ...
    const output = await run(["cargo", "+1.85.0", "run", "-p", packageName, "--offline", "--quiet"], workspace);
    ...
    const tree = await run(["cargo", "+1.85.0", "tree", "-p", packageName, "--offline", "--edges", "normal", "--prefix", "none"], workspace);
  ...
  const localOutput = await run(["cargo", "+1.85.0", "run", "-p", "longhorn-greenfield-optional-server-tauri", "--no-default-features", "--offline", "--quiet"], workspace);
  const localTrace = JSON.parse(localOutput.trim()) as Record<string, unknown>;
  const localTree = await run(["cargo", "+1.85.0", "tree", "-p", "longhorn-greenfield-optional-server-tauri", "--no-default-features", "--offline", "--edges", "normal", "--prefix", "none"], workspace);
