...
export async function packTypescriptArtifacts(repoRoot: string, artifactRoot: string) {
  ...
  for (const [name, directory] of typescriptPackages) {
    ...
    const path = join(artifactRoot, `${name.replace("@", "").replace("/", "-")}-0.2.0.tgz`);
...
longhorn-bridge = { path = "crates/longhorn-bridge", version = "0.2.0" }
longhorn-operation = { path = "crates/longhorn-operation", version = "0.2.0" }
longhorn-notifications = { path = "crates/longhorn-notifications", version = "0.2.0" }
longhorn-tauri-operation = { path = "crates/longhorn-tauri-operation", version = "0.2.0" }
longhorn-tauri-notifications = { path = "crates/longhorn-tauri-notifications", version = "0.2.0" }
