...
export async function packAndRunRustArtifacts(repoRoot: string, proofRoot: string, temporaryRoot: string) {
  ...
  for (const shape of shapes) {
    nativeTraces[shape] = parseTrace(await run(["cargo", "+1.85.0", "run", "-p", packages[shape], "--offline", "--quiet"], workspace));
    const tree = await run(["cargo", "+1.85.0", "tree", "-p", packages[shape], "--offline", "--prefix", "none"], workspace);
